...
var sessions = {}, timeout, cleanHandler = null;

exports.lookupOrCreate = lookupOrCreate;

// this should not normally be used, but it's there if you want to access the
// session store from a debugger
...
function lookupOrCreate(req, opts) {
	...
	// otherwise create a new session object with that ID, and store it
		...
		sess = new Session(id, opts);
		sessions[id] = sess;
	}

	// set the time at which the session can be reclaimed
	sess.expiration = (+new Date) + sess.lifetime * 1000;
	...
	for (var id in sessions)
		if (ownProp(sessions, id)) {
			if (sessions[id].expiration < now) {
				if (cleanHandler) cleanHandler(sessions[id]);
				delete sessions[id];
			} else
				next = next < sessions[id].expiration ? next
						: sessions[id].expiration;
		}
	if (next < Infinity)
...
function idFromRequest(req, opts) {
	...
	if (req.headers.cookie && (m = /FALLENSID=([^ ,;]*)/.exec(req.headers.cookie))
...
function Session(id, opts) {
	...
	// if the caller provides an explicit lifetime, then we use a persistent
	// cookie
	// it will expire on both the client and the server lifetime seconds after
	// the last use
	// otherwise, the cookie will exist on the browser until the user closes the
	// window or tab,
...
// randomString returns a pseude-random ASCII string which contains at least the
// specified number of bits of entropy
// the return value is a string of length [bits\6] of characters from the base64
// alphabet
function randomString(bits) {
	var chars, rand, i, ret;
	chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
	ret = '';
	// in v8, Math.random() yields 32 pseudo-random bits (in spidermonkey it
	// gives 53)
	while (bits > 0) {
		rand = Math.floor(Math.random() * 0x100000000); // 32-bit integer
		// base 64 means 6 bits per character, so we use the top 30 bits from
		// rand to give 30/6=5 characters.
...
// from milliseconds since the epoch to Cookie 'expires' format which is Wdy,
...
function dateCookieString(ms) {
	...
	mon = [ 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep',
			'Oct', 'Nov', 'Dec' ];
	return wdy[d.getUTCDay()] + ', ' + pad(d.getUTCDate()) + '-'
...
//Session.prototype.serialize // unimplemented, but would store the session on
							// disk
