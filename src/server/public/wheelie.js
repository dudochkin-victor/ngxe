(function() {
	...
	function drawWheel(x, y, rotation) {
		if (wheel && wheel.complete) {
			...
			try {
				...
				// draw the wheel image (taking into account the wheel image
				// size)
	...
	function animate() {
		...
		try {
			if (endPos && car && car.complete) {
				...
				var elapsedTime = new Date().getTime() - animationStart, carX = PROWEBAPPS.Easing.Back
						.Out(elapsedTime, 0, endPos.x, ANIMATION_DURATION)
						- car.width, wheelAngle = PROWEBAPPS.Easing.Back.Out(
	...
	$(document).ready(function() {
		...
		document.body.addEventListener("touchstart", function(evt) {
			startCar(evt.touches[0].pageX, evt.touches[0].pageY);
