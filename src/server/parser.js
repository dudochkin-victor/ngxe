...
var parseHtml = function(xml, callback) {
	...
	var saxParser = new libxml.SaxParser(function(parser) {
		...
		parser.onStartElementNS(function(elem, attrs, prefix, uri, namespaces) {
			if (elem.toLowerCase() != 'body') {
				// util.puts('START '+elem.toLowerCase());
				var id = null;
				properties = {};
				...
				for ( var idx = 0; idx < attrs.length; idx++) {
					...
					case 'style': {
						var style = {};
						var data = attrs[idx][3].replace(' ', '').split(';');
		...
		parser.onEndElementNS(function(elem, prefix, uri) {
			if (elem.toLowerCase() != 'body') {
				// util.puts('END '+elem.toLowerCase());
				properties = null;
				if (uistack.length == 1)
...
var callback = function(error, method, params) {
	if (error)
		util
				.puts('ERROR: '
						+ error.replace(/^\s\s*/, '').replace(/\s\s*$/, ''));
	else {
...
if (arguments.length > 0) {
	var page = fs.readFileSync(arguments[0], 'utf-8').toString()
	var body = /<body[\s\S]*>([\s\S]*)<\/body>/img.exec(page)[0];
	// .replace(/\s\s*/img, ' ')
	parseHtml(body, callback);
} else {
	util.puts('Usage : node parser file.html');
}