...
function check_conditions(object, condition) {
	if (object) {
		...
		case 'ANSWER': {
			...
			if ((!(val === NULL))
					&& (parseInt(val, 10) == parseInt(condition['val'], 10)))
				return register;
			break;
		}
			;
...
function handle(applogics, context, response, server_cmd) {
	...
	// data['gametime'] = 0;
	// } else {
	// difftime = (time - parseInt(data['lastvisit'], 10));
	// if (difftime < 100) {
	// gametime = difftime + parseInt(data['gametime'], 10);
	// data['gamehours'] = parseInt(gametime / 3600, 10);
	// data['gametime'] = parseInt(gametime, 10);
	// }
	// }
	// data['lastvisit'] = time;
