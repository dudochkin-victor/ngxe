...
class HMatrix {
	...
	static const HMatrix& IDENTITY() { static HMatrix* m=new HMatrix(Vector3<T>::X(),Vector3<T>::Y(),Vector3<T>::Z(),Vector3<T>::IDENTITY()); return *m; }

  // Creates the multiplicative identity matrix by default.
	HMatrix(const Vector3<T>& n=Vector3<T>::X(),const Vector3<T>& o=Vector3<T>::Y(),const Vector3<T>& a=Vector3<T>::Z(),const Vector3<T>& p=Vector3<T>::IDENTITY())
	...
	void set(const Vector3<T>& n,const Vector3<T>& o,const Vector3<T>& a,const Vector3<T>& p) { setN(n); setO(o); setA(a); setP(p); }

	// Accesses the "normal" vector component of this matrix.
	Vector3<T>& getN() { return m_n; }
	// Returns the "normal" vector component of this matrix.
	const Vector3<T>& getN() const { return m_n; }
	// Sets the "normal" vector component of this matrix.
	void setN(const Vector3<T>& n) { m_n=n;	}
	// Accesses the "open" vector component of this matrix.
	Vector3<T>& getO() { return m_o; }
	// Returns the "open" vector component of this matrix.
	const Vector3<T>& getO() const { return m_o; }
	// Sets the "open" vector component of this matrix.
	void setO(const Vector3<T>& o) { m_o=o;	}
	// Accesses the "approach" vector component of this matrix.
	Vector3<T>& getA() { return m_a; }
	// Returns the "approach" vector component of this matrix.
	const Vector3<T>& getA() const { return m_a; }
	// Sets the "approach" vector component of this matrix.
	void setA(const Vector3<T>& a) { m_a=a;	}
	// Accesses the "position" vector component (translation part) of this matrix.
	Vector3<T>& getP() { return m_p; }
	// Returns the "position" vector component (translation part) of this matrix.
	const Vector3<T>& getP() const { return m_p; }
	// Sets the "position" vector component (translation part) of this matrix.
	void setP(const Vector3<T>& p) { m_p=p;	}

	// Unary sign, returns a matrix unmodified.
	friend HMatrix& operator+(const HMatrix& m) { return m; }
	// Unary sign, returns the reverse of a matrix.
	friend HMatrix operator-(const HMatrix& m) { return HMatrix(-m.m_n,-m.m_o,-m.m_a,-m.m_p); }

  // Adds the components of two matrices.
	friend HMatrix operator+(const HMatrix& m,const HMatrix& l) { return HMatrix(m.m_n+l.m_n,m.m_o+l.m_o,m.m_a+l.m_a,m.m_p+l.m_p); }
	// Subtracts the components of two matrices.
	friend HMatrix operator-(const HMatrix& m,const HMatrix& l) { return HMatrix(m.m_n-l.m_n,m.m_o-l.m_o,m.m_a-l.m_a,m.m_p-l.m_p); }
	// Adds a matrix to this  matrix.
  const HMatrix& operator+=(const HMatrix& m) { m_n+=m.m_n; m_o+=m.m_o; m_a+=m.m_a; m_p+=m.m_p; return *this; }
  ...
  const HMatrix& operator*=(const HMatrix& m) { return *this=(*this)*m; }
  ...
  friend HMatrix operator~(const HMatrix& m);
	...
	static HMatrix createTranslation(const Vector3<T>& direction,double distance) { return HMatrix(Vector3<T>::X(),Vector3<T>::Y(),Vector3<T>::Z(),distance*(~direction)); }
	// Creates a translation matrix along the x-axis and the given distance.
	static HMatrix createTranslationX(T distance) { return createTranslation(Vector3<T>::X(),distance); }
	// Creates a translation matrix along the y-axis and the given distance.
	static HMatrix createTranslationY(T distance) { return createTranslation(Vector3<T>::Y(),distance); }
	// Creates a translation matrix along the z-axis and the given distance.
	static HMatrix createTranslationZ(T distance) { return createTranslation(Vector3<T>::Z(),distance); }
...
HMatrix<T> operator*(const HMatrix<T>& m,const HMatrix<T>& l) {
	...
	for (int row=2;row>=0;--row) {
		...
		p[row] = m[row]*l[12] + m[row+4]*l[13] + m[row+8]*l[14] + m[row+12];
	}
...
HMatrix<T> HMatrix<T>::operator!() const {
	return HMatrix(
		Vector3<T>(getN().getX(),getO().getX(),getA().getX()),
...
HMatrix<T> HMatrix<T>::createRotation(const Vector3<T>& axis,double angle) {
	...
	return HMatrix(
		Vector3<T>(1-c + axis.getX()*xc,  zs + axis.getY()*xc, -ys + axis.getZ()*xc),
		Vector3<T>(-zs + axis.getX()*yc, 1-c + axis.getY()*yc,  xs + axis.getZ()*yc),
		Vector3<T>( ys + axis.getX()*zc, -xs + axis.getY()*zc, 1-c + axis.getZ()*zc)
	);
}
...
void HMatrix<T>::setRotationCenter(const Vector3<T>& center) {
	setP(Vector3f(
		 center.getX()               - center.getX()*getN().getX() - center.getY()*getO().getX() - center.getZ()*getA().getX(),
	  -center.getX()*getN().getY() + center.getY()               - center.getY()*getO().getY() - center.getZ()*getA().getY(),
		-center.getX()*getN().getZ() - center.getY()*getO().getZ() + center.getZ()               - center.getZ()*getA().getZ()
	));
}
