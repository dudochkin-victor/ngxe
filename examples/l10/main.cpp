...
Bool loadGLTextures()   /* Load Bitmaps And Convert To Textures */
{
    ...
    if (loadBMP((char*)"../textures/mud.bmp", texti))
    {
        status = True;
...
GLvoid setupWorld()
{
    ...
    sector1.t = (triangle*)malloc (numTris * sizeof(triangle));
    ...
    for (triLoop = 0; triLoop < sector1.numTris; triLoop++)
    {
        for (vertLoop = 0; vertLoop < 3; vertLoop++)
        {
            ...
            sector1.t[triLoop].v[vertLoop].x = x;
            sector1.t[triLoop].v[vertLoop].y = y;
            sector1.t[triLoop].v[vertLoop].z = z;
            sector1.t[triLoop].v[vertLoop].u = u;
            sector1.t[triLoop].v[vertLoop].v = v;
        }
    }
    fclose(filein);
    return;
...
void keyAction(void)
{
    ...
    if (keys[keyCodes[1]])
    {
        killGLWindow();
        GLWin.fs = !GLWin.fs;
        createGLWindow((char*)"NeHe's 3D-World Tutorial",
            640, 480, 24, GLWin.fs);
