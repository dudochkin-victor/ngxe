...
int main()
{
  cout << "This is Tut1 example running" << endl;
  ...
  SDLVideoclass *SDLmain = new SDLVideoclass(640,480);
  //wanna have fullscreen?
  //SDLmain->Fullscreen();
  //find the closest actual resolution available:
  // SDLmain->FindMode();

  //ok lets get some going:
  ...
  //step 3: some primitive graphics on the screen.

  //clear the screen.
  glClearColor(0.0,0.0,0.4,0.0); //select a dark blue colour for clearing the screen.
  glClear(GL_COLOR_BUFFER_BIT); //tell opengl to clear the screen using the specified color.

  //Some matrix setups (dont worry about this too much for now)
  glMatrixMode(GL_PROJECTION);


  gluPerspective(45.0f,1.33,0.1f,100.0f);   //setup the camera
  //We use a Fov angle of 45 degrees. 
  //The aspectratio is set to 1.33 (640/480)
  //Everything closer than 0.1 is invisible and objects farther than 100 will also dissapear

  glMatrixMode(GL_MODELVIEW);//setup the way the geometry is altered
  glLoadIdentity();

  glShadeModel(GL_SMOOTH); //this will give our triangle nice blended colours
  //draw a triangle on the screen.
  glBegin(GL_TRIANGLES);   
  //3 vertices with each a different colour
  ...
  //done with triangle drawing:
  glEnd();

  //swap the front and back buffers so we can actually see what we've done
  SDL_GL_SwapBuffers();

  SDL_Delay(1000); //Delay 1 sec before exiting the application
  
  delete SDLmain;//cleanup
