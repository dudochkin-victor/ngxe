...
Texture *TexGen::Make(const unsigned char *data, bool allowMipMaps = true)
{
	...
	// Clear the layers
	memset(layers[0], 0,  sizeof(pixel)*256*256);
	memset(layers[1], 0, sizeof(pixel)*256*256);
	memset(layers[2], 0, sizeof(pixel)*256*256);
	memset(layers[3], 0, sizeof(pixel)*256*256);
