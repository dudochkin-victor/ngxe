...
Bool loadGLTextures()   /* Load Bitmaps And Convert To Textures */
{
    ...
    if (loadBMP((char*)"../textures/NeHe.bmp", texti))
    {
        status = True;
...
void keyPressed(KeySym key)
{
    switch (key)
    {
        ...
        case XK_F1:
            killGLWindow();
            GLWin.fs = !GLWin.fs;
            createGLWindow((char*)"NeHe's Texture Mapping Tutorial",
                640, 480, 24, GLWin.fs);
            break;
    }
}
...
int main(int argc, char **argv)
{
    ...
    GLWin.fs = True;
    if (!createGLWindow((char*)"NeHe's Texture Mapping Tutorial", 640, 480, 24,
        GLWin.fs))
    {
        done = True;
    }
