...
int LoadSprite(SDL_Surface *screen, char *file)
{
	...
	{
		SDL_SetColorKey(sprite, SDL_SRCCOLORKEY,
				SDL_MapRGB(sprite->format,0xff,0xff,0xff));
	}
...
{
	SDL_Surface *screen = this->GetSurface();
	...
	// Load the sprite
	if ( LoadSprite(screen, (char*)"icon.bmp") < 0 ) {
...
{
	Uint32 ticks, lastticks;
	// Отображаем нашу графику

	lastticks = SDL_GetTicks();
	// Update the frame -- move the sprite
	if ( mouse_pressed ) {
		MoveSprite(this->GetSurface(), light);
		mouse_pressed = 0;
	} else {
		MoveSprite(this->GetSurface(), NULL);
	}
				...
				const int& iRelY)
{
	...
	if ( iButton == 1 ) {
		...
		area.w = 32;
		area.h = 32;
		SDL_FillRect(this->GetSurface(), &area, 0);
		SDL_UpdateRects(this->GetSurface(),1,&area);
	}
}
