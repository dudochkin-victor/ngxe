...
typedef struct {
	...
	Window win;
	GLXContext ctx;
	XSetWindowAttributes attr;Bool fs;
...
int TestIntersionCylinder(const Cylinder& cylinder, const TVector& position,
		const TVector& direction, double& lamda, TVector& pNormal,
		TVector& newposition);
...
int DrawGLScene()	            // Here's Where We Do All The Drawing
	...
	if (hook_toball1) {
		...
		gluLookAt(pos.X(), pos.Y(), pos.Z(), pos.X() + dir.X(),
	...
	for (i = 0; i < 20; i++) {
		if (ExplosionArray[i]._Alpha >= 0) {
			...
			glTranslatef(
					(float) ExplosionArray[i]._Position.X()
							/ ExplosionArray[i]._Scale,
...
/* FIXME: bits is currently unused */Bool CreateGLWindow(const char* title, int width,
		int height, int bits, Bool fullscreenflag) {
	XVisualInfo *vi;
	Colormap cmap;
	...
	if (GLWin.fs) {
		...
		XSetStandardProperties(GLWin.dpy, GLWin.win, title, title, None, NULL,
				0, NULL);
		XMapRaised(GLWin.dpy, GLWin.win);
	}
...
void KeyPressed(KeySym key) {
	...
	case XK_F1:
		KillGLWindow();
		GLWin.fs = !GLWin.fs;
		CreateGLWindow("NeHe's Texture Mapping Tutorial", 640, 480, 24,
				GLWin.fs);
		break;
	case XK_Up:
		pos += TVector(0, 0, -10);
...
/*************************************************************************************/
/*************************************************************************************/
int FindBallCol(TVector& point, double& TimePoint, double Time2, int& BallNr1,
		int& BallNr2) {
	TVector RelativeV;
	TRay rays;
...
void idle() {
	...
	while (RestTime > ZERO) {
		...
		if (lamda != 10000) {
			...
			ArrayVel[BallNr] = TVector::unit(
					(normal * (2 * normal.dot(-ArrayVel[BallNr])))
							+ ArrayVel[BallNr]);
...
int TestIntersionCylinder(const Cylinder& cylinder, const TVector& position,
	...
	if (d <= cylinder._Radius) {
		...
		O.unit();
		s = fabs(
				sqrt(cylinder._Radius * cylinder._Radius - d * d)
						/ direction.dot(O));
