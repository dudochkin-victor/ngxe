...
void Update(float milliseconds) {
	...
	if (((tex_data[(((mx + 2) + (width * my)) * 3)] == 255) || mx > (width - 4))
			&& ((tex_data[(((mx - 2) + (width * my)) * 3)] == 255) || mx < 2)
			&& ((tex_data[((mx + (width * (my + 2))) * 3)] == 255)
					|| my > (height - 4))
			&& ((tex_data[((mx + (width * (my - 2))) * 3)] == 255) || my < 2)) {
...
void drawGLScene() {
	...
	for (loop = 0; loop < 4; loop++) /* Loop To Draw Our 4 Views */
	{
		...
		if (loop == 1) /* If We Are Drawing The Second Scene */
		{
			...
			gluPerspective(45.0, (GLfloat) (width) / (GLfloat) (height), 0.1f,
					500.0);
		}
		...
		if (loop == 2) /* If We Are Drawing The Third Scene */
		{
			...
			glViewport(window_width / 2, 0, window_width / 2,
					window_height / 2);
			glMatrixMode(GL_PROJECTION); /* Select The Projection Matrix */
			glLoadIdentity(); /* Reset The Projection Matrix */
			/* Set Up Perspective Mode To Fit 1/4 The Screen (Size Of A Viewport) */
...
{
	tex_data = (GLubyte*)malloc(width * height * 3); /* Allocate Space For Our Texture */
